...
Clay_WebGPU_Context *Clay_WebGPU_Initialize(WGPUDevice device, WGPUQueue queue,
    ...
    // 创建独立的文本渲染器
    context->textRenderer = text_renderer_create(device, queue, screenWidth, screenHeight);
    ...
    pipelineDesc.primitive = (WGPUPrimitiveState){
    ...
    context->rectanglePipeline = wgpuDeviceCreateRenderPipeline(device, &pipelineDesc);

    // 创建矩形渲染缓冲区（支持多个矩形批处理）
    context->vertexBuffer = wgpuDeviceCreateBuffer(device, &(WGPUBufferDescriptor){
        .label = {.data = "Rectangle Vertex Buffer", .length = WGPU_STRLEN},
        .usage = WGPUBufferUsage_Vertex | WGPUBufferUsage_CopyDst,
        .size = 1000 * 6 * 6 * sizeof(float), // 支持1000个矩形，每个6顶点，6浮点数(2位置+4颜色)
        .mappedAtCreation = false
    });

    context->indexBuffer = wgpuDeviceCreateBuffer(device, &(WGPUBufferDescriptor){
    ...
    printf("Clay WebGPU渲染器初始化成功\n");
    return context;
}

bool Clay_WebGPU_LoadFont(Clay_WebGPU_Context *context, const char *fontPath, int fontSize) {
    if (!context || !context->textRenderer) return false;

    int fontId = text_renderer_load_font(context->textRenderer, fontPath, fontSize);
    if (fontId < 0) {
        printf("字体加载失败: %s\n", fontPath);
        return false;
    }
...
void Clay_WebGPU_UpdateScreenSize(Clay_WebGPU_Context *context, uint32_t screenWidth, uint32_t screenHeight) {
    if (!context) return;
    ...
    if (context->textRenderer) {
        text_renderer_update_screen_size(context->textRenderer, screenWidth, screenHeight);
    }
}
...
void Clay_WebGPU_RenderText(Clay_WebGPU_Context *context, WGPURenderPassEncoder renderPass,
    ...
    if (!context || !context->textRenderer || !renderPass || !textData) return;

    text_renderer_render_clay_text(context->textRenderer, renderPass, textData, bbox);
}

void Clay_WebGPU_PrintTextStats(Clay_WebGPU_Context *context) {
    if (!context || !context->textRenderer) return;
...
void Clay_WebGPU_Render(Clay_WebGPU_Context *context, Clay_RenderCommandArray renderCommands) {
    if (!context) return;

    static int frame_count = 0;
    frame_count++;
    
    printf("=== 开始渲染帧 %d，总共 %d 个渲染命令 ===\n", frame_count, renderCommands.length);
    ...
    for (uint32_t i = 0; i < renderCommands.length; i++) {
        ...
        printf("处理渲染命令 %d，类型: %d\n", i, renderCommand->commandType);

        switch (renderCommand->commandType) {
            case CLAY_RENDER_COMMAND_TYPE_RECTANGLE: {
                rectangle_count++;
                Clay_RectangleRenderData *rectangleData = &renderCommand->renderData.rectangle;
                Clay_BoundingBox bbox = renderCommand->boundingBox;

                printf("矩形 #%d: 原始颜色RGBA(%.0f,%.0f,%.0f,%.0f)\n", 
                       rectangle_count,
                ...
                float x2 = ((bbox.x + bbox.width) / (float)context->screenWidth) * 2.0f - 1.0f;
                float y2 = 1.0f - ((bbox.y + bbox.height) / (float)context->screenHeight) * 2.0f;
                ...
                if (bbox.width <= 0 || bbox.height <= 0) {
                    printf("警告：矩形 #%d 尺寸无效 (%.1fx%.1f)，跳过渲染\n", 
                           rectangle_count, bbox.width, bbox.height);
                    break;
                }

                if (a <= 0.0f) {
                    printf("警告：矩形 #%d 透明度为0，跳过渲染\n", rectangle_count);
                    break;
                }

                // 检查批处理是否还有空间（每个矩形6个顶点，每个顶点6个float）
                if (rectangleBatch.vertex_count + 36 > 6000) {
                    printf("警告：矩形批处理已满，跳过剩余矩形\n");
                    break;
                }
                ...
                // 三角形1 (左上, 右上, 左下)
                vertices[0] = x1; vertices[1] = y1; vertices[2] = r; vertices[3] = g; vertices[4] = b; vertices[5] = a;
                vertices[6] = x2; vertices[7] = y1; vertices[8] = r; vertices[9] = g; vertices[10] = b; vertices[11] = a;
                vertices[12] = x1; vertices[13] = y2; vertices[14] = r; vertices[15] = g; vertices[16] = b; vertices[17] = a;
                
                // 三角形2 (右上, 右下, 左下)
                vertices[18] = x2; vertices[19] = y1; vertices[20] = r; vertices[21] = g; vertices[22] = b; vertices[23] = a;
                vertices[24] = x2; vertices[25] = y2; vertices[26] = r; vertices[27] = g; vertices[28] = b; vertices[29] = a;
                vertices[30] = x1; vertices[31] = y2; vertices[32] = r; vertices[33] = g; vertices[34] = b; vertices[35] = a;
                
                rectangleBatch.vertex_count += 36;
                
                printf("+ 矩形 #%d 已添加到批处理: 位置(%.1f,%.1f) 尺寸(%.1fx%.1f) NDC(%.3f,%.3f-%.3f,%.3f) 颜色(%.2f,%.2f,%.2f,%.2f)\n",
                       rectangle_count, bbox.x, bbox.y, bbox.width, bbox.height, 
                       x1, y1, x2, y2, r, g, b, a);
                break;
            }
            ...
            case CLAY_RENDER_COMMAND_TYPE_BORDER: {
                ...
                if (borderData->width.top > 0) {
                    ...
                    float y1 = 1.0f - (bbox.y / (float)context->screenHeight) * 2.0f;
                    float x2 = ((bbox.x + bbox.width) / (float)context->screenWidth) * 2.0f - 1.0f;
                    float y2 = 1.0f - ((bbox.y + borderData->width.top) / (float)context->screenHeight) * 2.0f;
                    ...
                    wgpuQueueWriteBuffer(context->queue, context->vertexBuffer, 0, vertices, sizeof(vertices));
                    wgpuRenderPassEncoderSetPipeline(renderPass, context->rectanglePipeline);
                    wgpuRenderPassEncoderSetVertexBuffer(renderPass, 0, context->vertexBuffer, 0, sizeof(vertices));
    ...
    if (rectangleBatch.vertex_count > 0) {
        printf("开始批处理渲染 %d 个矩形 (%d 个顶点)\n", 
               rectangleBatch.vertex_count / 36, rectangleBatch.vertex_count / 6);
        
        wgpuQueueWriteBuffer(context->queue, context->vertexBuffer, 0, 
                           rectangleBatch.vertices, rectangleBatch.vertex_count * sizeof(float));
        wgpuRenderPassEncoderSetPipeline(renderPass, context->rectanglePipeline);
        wgpuRenderPassEncoderSetVertexBuffer(renderPass, 0, context->vertexBuffer, 0, 
                                           rectangleBatch.vertex_count * sizeof(float));
        wgpuRenderPassEncoderDraw(renderPass, rectangleBatch.vertex_count / 6, 1, 0, 0);
    ...
    // 刷新任何剩余的文本批次
    printf("准备刷新文本批次...\n");
    if (context->textRenderer && context->textRenderer->current_batch.char_count > 0) {
        printf("检测到 %d 个字符在文本批次中，准备渲染...\n", 
    ...
    printf("=== 渲染帧 %d 完成，共渲染 %d 个矩形 ===\n", frame_count, rectangle_count);
...
void Clay_WebGPU_Cleanup(Clay_WebGPU_Context *context) {
    ...
    // 清理缓冲区
    if (context->vertexBuffer) wgpuBufferRelease(context->vertexBuffer);
    if (context->indexBuffer) wgpuBufferRelease(context->indexBuffer);
    if (context->uniformBuffer) wgpuBufferRelease(context->uniformBuffer);

    // 清理渲染管线
    if (context->rectanglePipeline) wgpuRenderPipelineRelease(context->rectanglePipeline);

    free(context);
    printf("Clay WebGPU渲染器已清理\n");
}
